(function () {
  ...
  // Blackout plates use the existing Zepto artwork and the same production
  // text geometry as the other single/two-line plate designs.
  CL_PLATE_CFG['california-blackout-plate-hat'] = {
    img: 'California-Blackout-License-Plate-Preview.jpg',
    f: {
  ...
  // Swap the GemPages hero heading with the collection's Custom Hero Text
  // metafield (emitted as window.CL_HERO_TEXT by the builder section). Targets a
  // [data-cl-hero] element if present, else the H1/H2 matching the default text.
  function applyHeroText() {
    var t = (typeof window !== 'undefined') ? window.CL_HERO_TEXT : null;
    var DEFAULT = 'The More You Bundle, The More You Save';
    if (!t || t === DEFAULT) return; // nothing to change
    var el = document.querySelector('[data-cl-hero], .cl-hero, #cl-hero');
    if (!el) {
      var hs = document.querySelectorAll('h1, h2');
      for (var i = 0; i < hs.length; i++) {
        if (hs[i].textContent.trim() === DEFAULT) { el = hs[i]; break; }
      }
    }
